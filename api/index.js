...
// IN VERCEL: We look ONE LEVEL UP for the frontend folder
app.use(express.static(path.join(__dirname, '../frontend')));

const GROQ_API_KEY = process.env.GROQ_API_KEY;
const ai = GROQ_API_KEY ? new Groq({ apiKey: GROQ_API_KEY }) : null;

// Auth Routes (Uses exported users array)
...
async function fetchStockNews() {
    try {
        ...
        while ((match = itemRegex.exec(xml)) !== null && items.length < 10) {
            ...
            items.push({ title: title.replace(/<!\[CDATA\[|\]\]>/g, ''), link, source });
        }
        newsCache = items;
...
app.get('/api/sentiment', async (req, res) => {
    ...
    if (!ai) {
        return res.json({ overallMood: 'neutral', moodScore: 50, aiPowered: false });
    }
    ...
    try {
        ...
        const response = await ai.chat.completions.create({
            ...
            messages: [{ role: 'system', content: 'Return JSON: { "overallMood": "bullish"|"bearish", "moodScore": 0-100 }' }, { role: 'user', content: headlines }],
            max_tokens: 150,
...
app.post('/api/vibe-trade', async (req, res) => {
    ...
    try {
        const response = await ai.chat.completions.create({
            model: 'llama-3.1-8b-instant',
            messages: [{ role: 'system', content: 'You are Labh Sathi.' }, { role: 'user', content: req.body.prompt }],
            max_tokens: 250
        });
